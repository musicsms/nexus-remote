...
| `nexus-client` | Native viewer/controller | **In progress** — signed capability/relay-token session lifecycle, authenticated bounded video/input receiver, depth-one render queue, portable input/window controllers, private Win32/D3D11/Media Foundation boundaries, bounded `ClientRuntime`, and synthetic QUIC loopback coverage are implemented. The entrypoint validates non-secret endpoint configuration and fails explicitly until authenticated control-plane bootstrap is wired; MSVC compilation, live Windows GUI/GPU/input smoke, and full host/client/service/relay acceptance remain unverified. |
