...
| `nexus-client` | Native viewer/controller | **In progress** — signed capability/relay-token session lifecycle, authenticated bounded video/input receiver, depth-one render queue, portable input/window controllers, private Win32/D3D11/Media Foundation boundaries, bounded `ClientRuntime`, reconnect retry/reset orchestration, and synthetic QUIC loopback coverage are implemented. The entrypoint validates endpoint configuration and can construct/run the runtime from explicitly supplied authenticated bootstrap metadata; it fails closed when that bootstrap is absent. MSVC compilation, live Windows GUI/GPU/input smoke, and full host/client/service/relay acceptance remain unverified. |
