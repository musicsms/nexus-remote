...
|---|---|---|
| `platform/windows/` | Windows-specific OS/codec bindings | **In progress** — portable capture/codec/input/cursor contracts and cfg-gated WGC→DXGI capture selection, DXGI capture, Media Foundation H.264, `SendInput`, and bounded cursor-capture implementations are present. Linux contract tests and `x86_64-pc-windows-gnu` `--tests` compilation pass; the installed toolchain lacks the MSVC target, and no interactive Windows hardware has verified WGC/DXGI capture, Media Foundation encoding, cursor capture, or input injection. |
